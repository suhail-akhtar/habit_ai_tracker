                ...
                object : RecognitionListener {
                    ...
                    override fun onBeginningOfSpeech() {
                        ...
                        channel.invokeMethod(
                                "onSpeechResult",
                                mapOf("recognizedWords" to "[Listening...]", "confidence" to 0.1)
                        )
                    }
                    ...
                    override fun onError(error: Int) {
                        ...
                        val shouldShowError =
                                when (error) {
                                    SpeechRecognizer.ERROR_NO_MATCH ->
                                            !hasReceivedSpeech // Only error if no speech detected
                        ...
                        if (shouldShowError) {
                            val errorMessage =
                                    when (error) {
                                        ...
                                        SpeechRecognizer.ERROR_RECOGNIZER_BUSY ->
                                                "Recognition service busy - please try again"
                                        SpeechRecognizer.ERROR_SERVER -> "Server error"
                                        SpeechRecognizer.ERROR_SPEECH_TIMEOUT ->
                                                "No speech detected - please try speaking"
                            ...
                            channel.invokeMethod(
                                    "onError",
                                    mapOf("errorType" to errorType, "message" to errorMessage)
                            )
                        } else {
                            // If we had speech but no match, just end gracefully
                            channel.invokeMethod(
                                    "onSpeechResult",
                                    mapOf("recognizedWords" to "", "confidence" to 0.0)
                            )
                        }
                    }
                    ...
                    override fun onPartialResults(partialResults: Bundle?) {
                        ...
                        if (!matches.isNullOrEmpty() && matches[0].isNotEmpty()) {
                            ...
                            channel.invokeMethod(
                                    "onSpeechResult",
                                    mapOf("recognizedWords" to matches[0], "confidence" to 0.5)
                            )
                        }
                    }
